...
public class ResourcePack {
	...
	*/
	...
	public void create(Video video) {
		...
		File assets = new File(resourcePackFolder + "/assets/minecraft/sounds/movietheatrecore/");
		assets.mkdirs();
		...
		try {
			...
			for(int i = 0; i < video.getAudioChannels(); i++) {
			    ...
			    inner.put("name", "movietheatrecore/" + i);
			    inner.put("preload", true);
				
			    param.add(inner);
			    
			    submab.put("sounds", param);
			    soundsMap.put("movietheatrecore." + i, submab);
	...
	* 75 is for versions 1.21.11
	* 
	* @return The resource pack-format.
	*/
	
	public int getResourcePackFormat() {
		if(plugin.getServerVersion().equals("v1_21_R7")) return 75;
