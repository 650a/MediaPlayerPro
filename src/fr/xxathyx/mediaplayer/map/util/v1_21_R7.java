...
/**
 * The v1_21_R7 class implements {@link MapUtil}, it can only be defined once if
...
public class v1_21_R7 implements MapUtil {
	...
	public void update(Player player, int id, byte[] buffer) {
		...
		try {
			...
			if(isPaper) {
		        ...
		        Class<?> craftPlayerClass = Class.forName("org.bukkit.craftbukkit.v1_21_R7.entity.CraftPlayer");
				...
				net.minecraft.server.network.PlayerConnection connection = ((net.minecraft.server.network.PlayerConnection) net.minecraft.server.level.EntityPlayer.class.getFields()[6].get(
						((org.bukkit.craftbukkit.v1_21_R7.entity.CraftPlayer)player).getHandle()));
